...
현재까지 소요 시간: **2시간 44분**  
모의 코딩테스트: AI 금지  
특이사항: 레거시 코드라서 그런지 테스트 코드에 문법 오류가 있음. 테스트 코드 실행 불가능.

## 구현할 기능 목록
### 입력 역할
- [x] 메뉴 추천을 받을 코치의 이름을 입력받는 기능
...
### Enums
- [x] 메뉴의 카테고리 (한식, 중식, 일식) 등의 열거형
- [x] 각 요일 정보를 열거형으로 관리
- [x] 메뉴 정보를 열거형으로 저장, 필드에는 카테고리가 포함되어 있어야 함.

### 추천 결과를 저장하는 역할
- [x] 각 요일, 코치별 점심 메뉴 추천 결과를 저장해야 함
아래같은 자료 구조를 사용하면 좋을 것 같음  
Map<코치, 추천된 메뉴> 를 저장한 맵이 Map<요일, ...> 에 들어가 있기

### 코치 역할
- [x] 코치의 이름이 필드에 있어야 함.
- [x] 못 먹는 음식 정보가 필드에 있어야 함.

### 도메인
- [x] 메뉴 목록을 셔플해서 메뉴를 뽑아야 함.
- [x] 만약 코치가 못먹는 음식이면 될때까지 다시 셔플 시도.
